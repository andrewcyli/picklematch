...
export const ScheduleView = ({
  ...
  if (currentView === 'player') {
      ...
      return <div className="h-full overflow-y-auto">
        ...
        <div className="px-2 space-y-2">
          {allPlayers.map(player => {
            ...
            const nextMatch = playerMatches.find(m => !matchScores.has(m.id));
            
            return (
              <Card key={player} className="p-3 cursor-pointer hover:bg-accent/50 transition-colors" onClick={() => setSelectedPlayer(player)}>
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <h3 className="font-semibold text-sm">{player}</h3>
                    {nextMatch && <p className="text-xs text-muted-foreground">
                      Next: Court {String.fromCharCode(64 + nextMatch.court)} • {new Date(Date.now() + nextMatch.startTime * 60000).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
                    </p>}
                  </div>
    ...
    // Player detail view
    const playerMatches = matches.filter(m => 
      m.team1.includes(selectedPlayer) || m.team2.includes(selectedPlayer)
    );
    const currentPlayerMatch = playerMatches.find(m => !matchScores.has(m.id));
    ...
    return <div className="h-full overflow-y-auto">
        ...
        <div className="flex items-center justify-between gap-2 px-2 pt-1">
          <div className="flex items-center gap-2 flex-1 min-w-0">
            ...
            <div className="flex-1 min-w-0">
              <h2 className="text-base font-bold text-foreground truncate">{selectedPlayer}</h2>
              <p className="text-[10px] text-muted-foreground">{playerMatches.length} total matches</p>
            </div>
          </div>
      ...
      <div className="px-2 pb-2 space-y-3">
        ...
        {currentPlayerMatch && (
          <div>
            <h3 className="text-xs font-semibold text-muted-foreground mb-2">NEXT MATCH</h3>
            <Card className="p-3 border-2 border-primary bg-primary/10">
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <Badge className="bg-primary text-primary-foreground text-xs">
                    Court {String.fromCharCode(64 + currentPlayerMatch.court)}
                  </Badge>
                  <div className="flex items-center gap-1 text-xs text-muted-foreground">
                    <Clock className="w-3 h-3" />
                    {new Date(Date.now() + currentPlayerMatch.startTime * 60000).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
                  </div>
                </div>
                
                <div className="flex items-center justify-between py-2">
                  <div className="font-semibold text-sm">
                    {currentPlayerMatch.team1.join(' & ')}
                  </div>
                  <div className="text-xs text-muted-foreground">VS</div>
                  <div className="font-semibold text-sm">
                    {currentPlayerMatch.team2.join(' & ')}
                  </div>
                </div>

                {courtElapsedTimes.get(currentPlayerMatch.court) !== undefined && (
                  <div className="flex items-center justify-center gap-1.5 py-1 px-2 rounded-lg bg-primary/20 border border-primary/30">
                    <Timer className="w-3 h-3 text-primary animate-pulse" />
                    <span className="text-xs font-bold text-primary">
                      In Progress: {formatTime(courtElapsedTimes.get(currentPlayerMatch.court) || 0)}
                    </span>
                  </div>
                )}
              </div>
        ...
        {upcomingMatches.length > 1 && (
          <div>
            <h3 className="text-xs font-semibold text-muted-foreground mb-2">UPCOMING ({upcomingMatches.length - 1})</h3>
            <div className="space-y-2">
              {upcomingMatches.slice(1).map(match => (
                <Card key={match.id} className="p-2.5 bg-accent/30">
                  <div className="flex items-center justify-between text-xs mb-1">
                    ...
                    <span className="text-muted-foreground flex items-center gap-1">
                      ...
                      {new Date(Date.now() + match.startTime * 60000).toLocaleTimeString('en-US', { hour: 'numeric', minute: '2-digit' })}
                    </span>
                  </div>
        ...
        {completedMatches.length > 0 && (
          <div>
            <h3 className="text-xs font-semibold text-muted-foreground mb-2">COMPLETED ({completedMatches.length})</h3>
            <div className="space-y-2">
              {completedMatches.map(match => {
                const score = matchScores.get(match.id);
                const isWinner = score && (
                  (match.team1.includes(selectedPlayer) && score.team1 > score.team2) ||
                  (match.team2.includes(selectedPlayer) && score.team2 > score.team1)
                );
                
                return (
                  <Card key={match.id} className={`p-2.5 ${isWinner ? 'bg-greenery/20 border-greenery/30' : 'bg-muted/40'}`}>
                    <div className="flex items-center justify-between text-xs mb-1">
                      <Badge variant="outline" className="text-[10px]">
                        Court {String.fromCharCode(64 + match.court)}
                      </Badge>
                      {isWinner && <Badge className="bg-greenery text-white text-[10px]">Won</Badge>}
                    </div>
                    <div className="flex items-center justify-between text-xs">
                      <span className="font-medium">{match.team1.join(' & ')}</span>
                      {score && (
